...
mod tests {
    ...
    fn synthetic_gray_masks_match_original_effects_and_clipping() {
        let fixture: serde_json::Value = serde_json::from_str(include_str!(
            "../../../docs/validation/text-pixels-probe.json"
        ))
        .unwrap();
        for case in fixture["cases"].as_array().unwrap() {
            let initial = bytes(case["initial"].as_str().unwrap());
            let pixels = SharedMemory::zeroed(initial.len()).unwrap();
            pixels.write(0, &initial).unwrap();
            let style = TextStyle {
                color: [220, 80, 30],
                edge_color: [40, 130, 240],
                shadow_color: [10, 20, 30],
                effects: case["effects"].as_u64().unwrap() as u32,
                opacity: case["opacity"].as_u64().unwrap() as u32,
                shadow_opacity: case["shadow_opacity"].as_u64().unwrap() as u32,
                ..Default::default()
            };
            let mask = Mask {
                offset: [0, 0],
                size: [3, 3],
                coverage: case["mask"]["coverage"]
                    .as_array()
                    .unwrap()
                    .iter()
            ...
            let position = [
                case["position"][0].as_i64().unwrap() as i32,
                case["position"][1].as_i64().unwrap() as i32,
            ];
            draw_mask(
                Canvas {
                    rgba: false,
            ...
            .unwrap();
            assert_eq!(
                pixels.read(0, initial.len()).unwrap(),
                bytes(case["expected"].as_str().unwrap()),
                "effect={:#x} opacity={} position={position:?}",
                style.effects,
                style.opacity
            );
            assert_eq!(
                case["final_opacity"].as_u64().unwrap() as u32,
                if style.effects & 1 != 0 {
                    style.shadow_opacity
                } else {
                    style.opacity
